...
const account1 = {
  ...
  transactions: [500.12, 250.15, -300, 5000.34, -850, -110, -170, 1100],
  interest: 1.5,
  pin: 1111,
  transactionsDates: [
    ...
    '2022-12-01T07:43:59.331Z',
    '2022-12-11T15:21:20.814Z',
  ],
  currency: 'USD',
  locale: 'en-US',
};
...
const formatTransactionsDate = function(date, locale) {
  ...
  if(dayPassed <= 7) return `${dayPassed} дней назад`
  else {
...
const createNicknames = function (accs) {
  accs.forEach(function (acc) {
    acc.nickName = acc.userName.toLocaleLowerCase().split(' ')
...
const displayTotal = function(account) {
  ...
  const withdralsTotal = account.transactions.filter(trans => trans < 0).reduce((acc, trans) => acc + trans, 0)
  labelSumOut.textContent = formatCurrency(withdralsTotal, account.locale, account.currency)
...
btnLogin.addEventListener('click', function (e) {
  ...
  if(currentAccount?.pin === +(inputLoginPin.value)) {
    ...
    // const noww = new Date()
    // const date = `${noww.getDate()}`.padStart(2, '0')
...
btnTransfer.addEventListener('click', function(e) {
  ...
  const ricipientNickname = inputTransferTo.value
  const ricipientAccount = accounts.find(account => account.nickName === ricipientNickname)
  

  inputTransferTo.value = ''
  inputTransferAmount.value = ''

  if(transferAmount > 0 && currentAccount.balance >= transferAmount && ricipientAccount &&currentAccount.nickName !== ricipientAccount?.nickName) {
    currentAccount.transactions.push(-transferAmount)
    ricipientAccount.transactions.push(transferAmount)
    currentAccount.transactionsDates.push(new Date().toISOString())
    ricipientAccount.transactionsDates.push(new Date().toISOString())
    updateUi(currentAccount)
...
btnClose.addEventListener('click', function(e) {
  ...
  if(inputCloseUsername.value === currentAccount.nickName && +(inputClosePin.value) === currentAccount.pin) {
